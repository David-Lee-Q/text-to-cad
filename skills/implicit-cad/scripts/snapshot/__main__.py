...
SNAPSHOT_ORIGIN = "http://implicit-cad-snapshot.local"
...
DEFAULT_RENDER_WIDTH = DIAGNOSTIC_RENDER_WIDTH
DEFAULT_RENDER_HEIGHT = DIAGNOSTIC_RENDER_HEIGHT
...
def help_text() -> str:
    return """Usage:
  python scripts/snapshot --input models/example.implicit.js --output /tmp/example.png
  python scripts/snapshot --input models/example.implicit.js --output /tmp/example.gif --mode orbit
  python scripts/snapshot --job render-job.json
  python scripts/snapshot --job - --json

Snapshot renders browser-native implicit CAD .implicit.js/.implicit.mjs modules. Supported shortcut flags are --input, --output/-o, --mode, --appearance, --camera, --size-profile, --width, --height, --params, --graphics, --job, and --json. The default appearance is the light workbench saved theme. --appearance accepts a saved theme name, an inline JSON appearance settings object, or a JSON appearance settings file path. --camera accepts a preset, azimuth:elevation pair, or JSON object with preset, position, target, up, direction, and zoom fields. JSON jobs use input, mode=view/orbit/animate, outputs, camera, width/height, appearance, graphics, implicitParameters, implicitAnimation, orbit, and optional render.transparent. Output file names are saved with a shared UTC seconds timestamp before the extension.
"""
...
def parse_params_option(raw_params: object) -> dict[str, object]:
    ...
    if not is_plain_object(parsed):
        raise SnapshotError("--params must be an implicit parameter JSON object")
    return dict(parsed)
...
def load_graphics_option(raw_graphics: object, *, cwd: Path) -> dict[str, object]:
    graphics = str(raw_graphics or "").strip()
    if not graphics:
        raise SnapshotError("--graphics requires a JSON object or JSON file path")
    if graphics.startswith("{"):
        return validate_direct_settings_payload(
            load_json_text(graphics, "--graphics"),
            option_name="--graphics",
            source_label="--graphics",
            allowed_keys=GRAPHICS_OPTION_KEYS,
            setting_label="implicit graphics settings",
        )

    graphics_path = Path(graphics) if Path(graphics).is_absolute() else cwd / graphics
    if not graphics_path.exists():
        raise SnapshotError(f"Graphics JSON file does not exist: {graphics}")
    return validate_direct_settings_payload(
        load_json_text(graphics_path.read_text(encoding="utf-8"), str(graphics_path)),
        option_name="--graphics",
        source_label=str(graphics_path),
        allowed_keys=GRAPHICS_OPTION_KEYS,
        setting_label="implicit graphics settings",
    )
...
def apply_option_overrides_to_job(job: object, options: SnapshotOptions, *, cwd: Path) -> object:
    ...
    if options.width:
        next_job["width"] = options.width
    if options.height:
        next_job["height"] = options.height
    ...
    if options.params_specified:
        next_job["implicitParameters"] = parse_params_option(options.params)
...
def load_job_from_options(
    ...
    *,
    ...
    if options.job:
        ...
        payload = load_json_text(text, source_label)
        if isinstance(payload, list):
            return [apply_option_overrides_to_job(job, options, cwd=resolved_cwd) for job in payload]
        if is_plain_object(payload) and isinstance(payload.get("jobs"), list):
            next_payload = copy.deepcopy(payload)
            next_payload["jobs"] = [apply_option_overrides_to_job(job, options, cwd=resolved_cwd) for job in payload["jobs"]]
            return next_payload
        return apply_option_overrides_to_job(payload, options, cwd=resolved_cwd)

    if not stdin.isatty() and not options.input:
        text = stdin.read()
        if text.strip():
            return load_json_text(text, "stdin")

    if not options.input:
        raise SnapshotError("render requires --job, stdin JSON, or --input")
    if not options.output:
        raise SnapshotError("render shortcut requires --output")
    ...
    if not mode:
        mode = "orbit" if options.output.lower().endswith(".gif") else "view"
    job = {
        "input": options.input,
        "mode": mode,
        "outputs": [output],
    ...
    if options.params_specified:
        job["implicitParameters"] = parse_params_option(options.params)
    if options.graphics_specified:
...
def is_implicit_path(path: Path) -> bool:
    name = path.name.lower()
    return name.endswith(".implicit.js") or name.endswith(".implicit.mjs")
...
def resolve_input_path(raw_input: object, *, cwd: Path) -> Path:
    ...
    if not is_implicit_path(selected):
        raise SnapshotError("Snapshot supports only .implicit.js and .implicit.mjs inputs")
    return selected
...
def default_render_size(job: Mapping[str, object], output: Mapping[str, object]) -> tuple[int, int]:
    ...
    if profile == "orbit" or mode in {"orbit", "animate"} or is_plain_object(job.get("implicitAnimation")):
        return ORBIT_RENDER_WIDTH, ORBIT_RENDER_HEIGHT
    if profile in {"dimensioned", "section", "labeled", "diagnostic"} or appearance_theme_id_for_job(job) in WORKBENCH_RENDER_THEME_IDS:
...
def resolve_render_job(
    ...
    *,
    ...
    if "params" in job:
        raise SnapshotError("render jobs use implicitParameters; params is reserved for shortcut --params parsing")
    mode = str(job.get("mode") or "view").strip().lower()
    if mode not in {"view", "orbit", "animate"}:
        raise SnapshotError("Implicit CAD snapshot supports only view, orbit, and animate modes")
    render_settings = dict(job.get("render") if is_plain_object(job.get("render")) else {})
    graphics_settings = dict(job.get("graphics") if is_plain_object(job.get("graphics")) else {})
    appearance_settings = job.get("appearance") or DEFAULT_RENDER_THEME_ID
    resolved_cwd = (cwd or Path.cwd()).resolve()
    input_path = resolve_input_path(job.get("input"), cwd=resolved_cwd)
    root_path = input_path.parent.resolve()
    outputs = job.get("outputs") if isinstance(job.get("outputs"), list) else []
    if not outputs:
        raise SnapshotError("render job must include outputs")
...
def max_output_size(job: Mapping[str, object]) -> tuple[int, int]:
    ...
    widths = [int(output.get("width") or DEFAULT_RENDER_WIDTH) for output in outputs if is_plain_object(output)]
    heights = [int(output.get("height") or DEFAULT_RENDER_HEIGHT) for output in outputs if is_plain_object(output)]
    return max(widths or [DEFAULT_RENDER_WIDTH]), max(heights or [DEFAULT_RENDER_HEIGHT])
...
class BatchSnapshotRenderer:
    ...
    async def start(self) -> None:
        ...
        try:
            try:
                ...
                raise SnapshotError(
                    "Implicit CAD snapshot requires the Python playwright package. "
                    "Install the skill requirements, then run `python -m playwright install chromium` if needed."
                ) from exc
    ...
    async def render(self, job: Mapping[str, object]) -> dict[str, object]:
        ...
        result = await with_snapshot_timeout(
            self.page.evaluate("(renderJob) => window.__implicitCadSnapshotRender(renderJob)", dict(job)),
            job.get("timeoutSeconds") or DEFAULT_TIMEOUT_SECONDS,
        )
...
async def render_resolved_job_packet(packet: Mapping[str, object]) -> dict[str, object]:
...
if __name__ == "__main__":
    raise SystemExit(run_render_cli(sys.argv[1:]))
