...
float planetarySunGear(vec3 p) {
  ...
  float bore = implicitCadCylinderCapped(
    p,
  ...
  return implicitCadIntersectRound(implicitCadUnionRound(gear, hub, gearModule * 0.18), -bore, gearModule * 0.12);
}
...
float planetaryPlanetGear(vec3 p, float index) {
  ...
  return implicitCadIntersectRound(implicitCadUnionRound(gear, pinBoss, gearModule * 0.15), -bore, gearModule * 0.1);
}

float planetaryCarrierArm(vec3 p, vec2 center) {
  float zCenter = -thickness * 0.72;
  float arm2 = implicitCadLineSegment2(p.xy, vec2(0.0), center) - gearModule * 0.95;
...
float planetaryCarrier(vec3 p) {
  ...
  for (int i = 0; i < 3; i += 1) {
    ...
    carrier = implicitCadUnionRound(carrier, arm, gearModule * 0.32);
    carrier = implicitCadUnionRound(carrier, pin, gearModule * 0.2);
  }
  return carrier;
}
...
vec3 planetaryPalette(float component, vec3 p, vec3 normal) {
  vec3 ring = vec3(0.12, 0.56, 1.0);
  vec3 sun = vec3(1.0, 0.76, 0.08);
  vec3 planetA = vec3(0.34, 1.0, 0.28);
  vec3 planetB = vec3(1.0, 0.33, 0.78);
  vec3 planetC = vec3(0.22, 0.92, 1.0);
  vec3 carrier = vec3(0.93, 0.50, 1.0);
  vec3 color = ring;
  if (component > 0.5) {
    color = sun;
  }
