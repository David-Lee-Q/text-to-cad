...
from cadgen._internal.generation_status import (  # noqa: E402
    GENERATION_LOCK_SUFFIX,
...
class GenerationLockPathTest(unittest.TestCase):
    def test_lock_is_a_hidden_sibling_of_the_package_dir(self):
        ...
        self.assertEqual(
            ...
            generation_lock_path(package),
        )

    def test_suffix_is_a_bare_sentinel_not_a_payload_file(self):
        # Nothing reads the contents; a .json suffix would imply otherwise.
        self.assertEqual(".generation.lock", GENERATION_LOCK_SUFFIX)
        self.assertFalse(GENERATION_LOCK_SUFFIX.endswith(".json"))

    def test_distinct_models_get_distinct_locks(self):
        base = Path("/m/__cadgen__/models")
        self.assertNotEqual(
            generation_lock_path(base / "a.step.py"), generation_lock_path(base / "b.step.py")
        )
...
class GenerationLockBehaviourTest(unittest.TestCase):
    def setUp(self):
        ...
        self.lock = generation_lock_path(self.root / "__cadgen__" / "models" / "part.step.py")

    def test_none_lock_path_is_a_noop(self):
        with track_generation_run(None):
            pass
    ...
    def test_lock_file_survives_release(self):
        ...
        with track_generation_run(self.lock):
            pass
    ...
    def test_different_models_do_not_contend(self):
        other = generation_lock_path(self.root / "__cadgen__" / "models" / "other.step.py")
        done = threading.Event()

        def run():
            with track_generation_run(other):
                done.set()
                time.sleep(0.05)
    ...
    def test_killed_holder_releases_the_lock(self):
        ...
        script = textwrap.dedent(
            ...
            from cadgen._internal.generation_status import track_generation_run
            with track_generation_run({str(self.lock)!r}):
    ...
    def test_skip_if_current_is_evaluated_under_the_lock(self):
        ...
        result = _run_with_spec_generation_status(
            ...
            lambda spec: calls.append("built"),
