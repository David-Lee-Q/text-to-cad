...
DOUBLE_SOURCE = V2_DIR / "assemblies" / "servo_horn_yoke_double.step.py"
...
# Resolve robot_common while V2_DIR is on sys.path. The cad CLI's generator loader
# restores sys.path after exec_module, so deferring this import to gen_step() would
# fail in a fresh CLI process — bind it at module load time instead.
...
def _shoulder_double_servo_yoke_mates(tom_module) -> list[dict[str, object]]:
    # Side-by-side servo <-> double-horn / double-mount mates. These are tom_double
    # TOP-LEVEL descriptive mates (relocated here so servo_horn_yoke_double stays a
    # joint-only sub-assembly); they get prefixed onto the shoulder link below.
    yoke = "servo_horn_yoke_double_horn"
    mount = "servo_end_mount_double"
    clearance = {"clearance_mm": 0.0}
    return [
        tom_module._mate(
...
def _replace_shoulder_double_mates(
    *,
    ...
    mates.extend(
        ...
        for mate in _shoulder_double_servo_yoke_mates(tom_module)
    )
    mates.append(
        tom_module._mate(
...
def gen_step_with_options(*, include_gripper: bool = False) -> dict[str, object]:
    ...
    double_envelope = double_module.assembly_instances()
...
def gen_urdf() -> dict[str, object]:
    ...
    robot.append(tom_module.ET.Comment(" Generated by models/robots/tom/tom_double.py. "))
    robot.append(
