...
// render-artifact state machine. The freshness/build internals stay exactly where they are
// (readStepSourceStatus / ensureStepTopologyArtifact / the cadpy step_artifact CLI).
//
 ...
 */
export function createStepArtifactProvider({ ownsEntry, readStatus, build, artifactRef } = {}) {
  ...
  return {
    ...
    // Freshness only — never builds. Maps the STEP artifact validation onto ready|needs-build|error.
    status({ fileRef, entry, resolvedRoot, catalog } = {}) {
      let status;
      try {
    ...
    // Idempotent (re)build via the existing route-level builder.
    async build({ fileRef, force = false, entry, resolvedRoot, catalog } = {}) {
      const result = await build({ fileRef, force, resolvedRoot, catalog });
      if (result?.ok) {
